...
try {
  ...
  regional = collected.filter(a => {
    ...
    const geoMatch = allowedGeo.some(g => text.includes(g));
    const entityMatch = regionalEntities.some(e => text.includes(e));

    return geoMatch || entityMatch;
  });

  // 🔁 Fallback – wenn nichts passt, nimm erste 3 Artikel
  if (regional.length === 0) {
    regional = collected.slice(0, 3);
  }

...
var require_pump = __commonJS({
  "../../node_modules/.pnpm/pump@3.0.4/node_modules/pump/index.js"(exports, module2) {
    ...
    var isFS = function(stream) {
      ...
      if (!fs19)
        return false;
      return (stream instanceof (fs19.ReadStream || noop) || stream instanceof (fs19.WriteStream || noop)) && isFn(stream.close);
    };
    var isRequest = function(stream) {
...
var require_operator = __commonJS({
  "../../node_modules/.pnpm/@renovatebot+pep440@4.2.1/node_modules/@renovatebot/pep440/lib/operator.js"(exports, module2) {
    var { parse: parse2 } = require_version();
    module2.exports = {
      compare,
      rcompare,
      lt,
    ...
    function compare(version2, other) {
      const parsedVersion = parse2(version2);
...
var require_specifier = __commonJS({
  "../../node_modules/.pnpm/@renovatebot+pep440@4.2.1/node_modules/@renovatebot/pep440/lib/specifier.js"(exports, module2) {
    ...
    function satisfies(version2, specifier, options = {}) {
    ...
    function contains(specifier, input) {
      ...
      if (spec.operator === "~=") {
        ...
        return satisfies(version2, `>=${spec.version}, ==${compatiblePrefix}`);
      }
      if (spec.prefix) {
    ...
    function validRange(specifier) {
      return Boolean(parse2(specifier));
    }
  }
});
...
var require_semantic = __commonJS({
  "../../node_modules/.pnpm/@renovatebot+pep440@4.2.1/node_modules/@renovatebot/pep440/lib/semantic.js"(exports, module2) {
    var { explain, parse: parse2, stringify } = require_version();
    module2.exports = {
      major,
      minor,
      patch,
...
async function getQueueAdapterBootstrap({
  ...
  for (const [upstream, adapter] of UPSTREAM_DEPENDENCY_ADAPTERS) {
    ...
    const name = adapter.preferUnbundledWhenPresent.some(
      (dep) => dependencies.has(dep)
    ) ? adapter.unbundled : adapter.bundled;
...
async function getDirectDependencyNames(pythonPackage) {
  ...
  const parsedDependencies = await (0, import_python_analysis.parsePep508)(dependencies);
  const dependencyNames = new Set(
    parsedDependencies.filter((dependency) => dependency !== null).map((dependency) => (0, import_python_analysis.normalizePackageName)(dependency.name))
  );
...
async function getDeclaredDependencyNames(dependencies) {
  ...
  const parsed = await (0, import_python_analysis2.parsePep508)(
    dependencies.filter((dep) => typeof dep === "string")
  );
  return new Set(
    parsed.filter((dep) => dep !== null).map((dep) => (0, import_python_analysis2.normalizePackageName)(dep.name))
  );
}
async function isLegacyWorkersProject(workPath) {
  const pyprojectPath = (0, import_path.join)(workPath, "pyproject.toml");
  if (!import_fs.default.existsSync(pyprojectPath)) {
    return false;
  }
  const pyproject = await (0, import_build_utils.readConfigFile)(pyprojectPath);
...
var UvRunner = class {
  ...
  async addDependencies(options) {
    ...
    (0, import_build_utils2.debug)(`Running "uv ${args.join(" ")}" in ${projectDir}...`);
    await this.runUvCmd(args, projectDir, venvPath);
  }
  ...
  async cachePrune() {
    ...
    try {
      await (0, import_execa2.default)(this.uvPath, args, {
  ...
  async runUvCmdWithOutput(args, cwd, venvPath, env) {
    const pretty = `uv ${args.join(" ")}`;
    (0, import_build_utils2.debug)(`Running "${pretty}"...`);
    try {
      const result = await (0, import_execa2.default)(this.uvPath, args, {
        cwd,
  ...
  getVenvEnv(venvPath) {
    const binDir = isWin ? (0, import_path2.join)(venvPath, "Scripts") : (0, import_path2.join)(venvPath, "bin");
    const existingPath = process.env.PATH || "";
    return {
      ...getProtectedUvEnv(process.env, this.uvCacheDir),
      VIRTUAL_ENV: venvPath,
      PATH: existingPath ? `${binDir}${import_path3.delimiter}${existingPath}` : binDir
    };
  }
};
...
async function getUvBinaryForBundling(pythonPath) {
  ...
  const resolvedPath = await import_fs2.default.promises.realpath(uvPath);
  return resolvedPath;
}
var UV_LINUX_TARGET = "x86_64-unknown-linux-gnu";
async function downloadUvBinaryForTarget(cacheDir) {
  const destDir = (0, import_path2.join)(cacheDir, `uv-${UV_VERSION}-${UV_LINUX_TARGET}`);
  const destBinary = (0, import_path2.join)(destDir, "uv");
  try {
    await import_fs2.default.promises.access(destBinary, import_fs2.default.constants.X_OK);
    (0, import_build_utils2.debug)(`Using cached uv binary at ${destBinary}`);
    return destBinary;
  } catch {
  }
  ...
  (0, import_build_utils2.debug)(`Downloading uv ${UV_VERSION} from ${url}`);
  console.log(
  ...
  try {
    await (0, import_execa2.default)("tar", [
      "xzf",
      tarballPath,
      "--strip-components=1",
      "-C",
  ...
  await import_fs2.default.promises.chmod(destBinary, 493);
  await import_fs2.default.promises.unlink(tarballPath).catch(() => {
  });
  (0, import_build_utils2.debug)(`Downloaded uv binary to ${destBinary}`);
  return destBinary;
}
async function downloadUvTarball(url, dest) {
  ...
  try {
    ...
    await (0, import_promises.pipeline)(
      ...
      import_fs2.default.createWriteStream(tmpDest)
    );
    await import_fs2.default.promises.rename(tmpDest, dest);
  } catch (err) {
    await import_fs2.default.promises.unlink(tmpDest).catch(() => {
    });
    if (err instanceof import_build_utils2.NowBuildError) {
      throw err;
    }
    throw new import_build_utils2.NowBuildError({
      code: "RUNTIME_DEPENDENCY_INSTALLATION_FAILED",
...
async function sha256File(filePath) {
  const data = await import_fs2.default.promises.readFile(filePath);
...
function getVenvBinDir(venvPath) {
  return (0, import_path4.join)(venvPath, isWin2 ? "Scripts" : "bin");
}
function useVirtualEnv(workPath, env, systemPython) {
  const venvDirs = [".venv", "venv"];
  let pythonCmd = systemPython;
  for (const venv of venvDirs) {
    const venvRoot = (0, import_path4.join)(workPath, venv);
    const binDir = process.platform === "win32" ? (0, import_path4.join)(venvRoot, "Scripts") : (0, import_path4.join)(venvRoot, "bin");
    const candidates = process.platform === "win32" ? [(0, import_path4.join)(binDir, "python.exe"), (0, import_path4.join)(binDir, "python")] : [(0, import_path4.join)(binDir, "python3"), (0, import_path4.join)(binDir, "python")];
    const found = candidates.find((p) => import_fs3.default.existsSync(p));
    if (found) {
      pythonCmd = found;
      env.VIRTUAL_ENV = venvRoot;
      env.PATH = `${binDir}${import_path4.delimiter}${env.PATH || ""}`;
...
function createVenvEnv(venvPath, baseEnv = process.env, uvCacheDir) {
  ...
  env.PATH = existingPath ? `${binDir}${import_path4.delimiter}${existingPath}` : binDir;
  return env;
}
...
async function ensureVenv({
  ...
  const marker = (0, import_path4.join)(venvPath, "pyvenv.cfg");
  let venvExists = false;
  try {
    await import_fs3.default.promises.access(marker);
    venvExists = true;
  } catch {
  }
  if (venvExists && pythonVersion.major != null && pythonVersion.minor != null) {
    ...
    if (cachedVersion && cachedVersion !== expected) {
      ...
      await import_fs3.default.promises.rm(venvPath, { recursive: true, force: true });
      venvExists = false;
    }
  }
  if (venvExists) {
    (0, import_build_utils3.debug)(`Refreshing cached virtual environment at "${venvPath}"`);
  } else {
    await import_fs3.default.promises.mkdir(venvPath, { recursive: true });
    if (!quiet) {
  ...
  if (uvPath) {
    ...
    await (0, import_execa3.default)(uvPath, args, {
      env: getProtectedUvEnv(process.env, uvCacheDir)
    });
  } else {
    await (0, import_execa3.default)(pythonVersion.pythonPath, ["-m", "venv", venvPath]);
  }
}
function getVenvPythonBin(venvPath) {
  return (0, import_path4.join)(getVenvBinDir(venvPath), isWin2 ? "python.exe" : "python");
}
async function runPyprojectScript(workPath, scriptNames, env, useUserVirtualEnv = true) {
  const pyprojectPath = (0, import_path4.join)(workPath, "pyproject.toml");
  if (!import_fs3.default.existsSync(pyprojectPath))
    return false;
  let pyproject = null;
  try {
    pyproject = await (0, import_build_utils3.readConfigFile)(pyprojectPath);
  } catch {
  ...
  if (typeof scriptCommand === "string" && scriptCommand.trim()) {
    ...
    await (0, import_build_utils3.execCommand)(scriptCommand, {
      cwd: workPath,
      env: finalEnv
    });
    return true;
...
function resolvePythonVersion({
  ...
  if (!constraints || constraints.length === 0) {
    ...
    const result = (0, import_python_analysis3.selectPythonVersion)({
      constraints,
      availableBuilds,
      allBuilds,
      defaultBuild,
  ...
  const hasPythonVersionFile = pythonPackage.configs?.some(
    (configSet) => configSet[import_python_analysis3.PythonConfigKind.PythonVersion] !== void 0
  ) ?? false;
  if (!hasPythonVersionFile && pythonPackage.manifest && versionLessOrEqual(selection, defaultPv)) {
    if (autoUpgraded || !pythonPackage.manifest.origin && source?.endsWith("pyproject.toml")) {
      const manifestDir = (0, import_path5.join)(rootDir, (0, import_path5.dirname)(pythonPackage.manifest.path));
      pinVersionFilePath = (0, import_path5.join)(manifestDir, ".python-version");
    }
  }
  return {
...
function getInstalledPythonsFromFilesystem(basePath = UV_PYTHON_PATH_PREFIX) {
  ...
  for (const opt of allOptions) {
    ...
    if (!version2)
      continue;
    const binPath = (0, import_path5.join)(basePath, "bin", `python${version2}`);
    if ((0, import_fs4.existsSync)(binPath)) {
      result.add(version2);
    }
  }
  return result;
...
async function restoreCachedLock(cachedLockPath, projectDir) {
  if (!cachedLockPath)
    return;
  const targetLockPath = (0, import_path6.join)(projectDir, "uv.lock");
  if (import_fs5.default.existsSync(cachedLockPath) && !import_fs5.default.existsSync(targetLockPath)) {
    (0, import_build_utils6.debug)("Restoring cached uv.lock");
    await import_fs5.default.promises.copyFile(cachedLockPath, targetLockPath);
  }
}
...
async function isInstalled2(pythonPath, dependency, cwd) {
  try {
    const { stdout } = await (0, import_execa4.default)(
      pythonPath,
      ["-c", makeDependencyCheckCode(dependency)],
      {
        stdio: "pipe",
        cwd,
        env: { ...process.env, PYTHONPATH: (0, import_path6.join)(cwd, resolveVendorDir()) }
      }
    );
...
`.trim();
  const { stdout } = await (0, import_execa4.default)(pythonBin, ["-c", code]);
  try {
    ...
    (0, import_build_utils6.debug)("Failed to parse site-packages output", err);
  }
  return [];
}
...
async function discoverPackage({
  ...
  try {
    return await (0, import_python_analysis4.discoverPythonPackage)({ entrypointDir, rootDir });
  } catch (error) {
    if (error instanceof import_python_analysis4.PythonAnalysisError) {
...
function detectInstallSource(pythonPackage, rootDir) {
  ...
  if (lockFile) {
    if (lockFile.kind === import_python_analysis4.PythonLockFileKind.UvLock) {
      manifestType = "uv.lock";
      manifestPath = (0, import_path6.join)(rootDir, lockFile.path);
    } else if (lockFile.kind === import_python_analysis4.PythonLockFileKind.PylockToml) {
      manifestType = "pylock.toml";
      manifestPath = (0, import_path6.join)(rootDir, lockFile.path);
    }
  } else if (pythonPackage.manifest) {
    manifestType = "pyproject.toml";
    manifestPath = (0, import_path6.join)(rootDir, pythonPackage.manifest.path);
  }
...
async function ensureUvProject({
  ...
  if (manifestType === "uv.lock" || manifestType === "pylock.toml") {
    ...
    lockPath = (0, import_path6.join)(rootDir, lockFile.path);
    projectDir = (0, import_path6.dirname)(lockPath);
    pyprojectPath = (0, import_path6.join)(projectDir, "pyproject.toml");
    if (!import_fs5.default.existsSync(pyprojectPath)) {
      throw new Error(
    ...
    projectDir = (0, import_path6.join)(rootDir, (0, import_path6.dirname)(manifest.path));
    pyprojectPath = (0, import_path6.join)(rootDir, manifest.path);
    const originKind = manifest.origin?.kind;
    if (originKind === import_python_analysis4.PythonManifestConvertedKind.Pipfile) {
      console.log("Installing required dependencies from Pipfile...");
    } else if (originKind === import_python_analysis4.PythonManifestConvertedKind.PipfileLock) {
      console.log("Installing required dependencies from Pipfile.lock...");
    } else if (originKind === import_python_analysis4.PythonManifestConvertedKind.RequirementsTxt || originKind === import_python_analysis4.PythonManifestConvertedKind.RequirementsIn) {
      console.log(
    ...
    if (manifest.origin) {
      ...
      pyprojectPath = (0, import_path6.join)(projectDir, "pyproject.toml");
      await import_fs5.default.promises.writeFile(pyprojectPath, content);
    }
    const workspaceLockFile = pythonPackage.workspaceLockFile;
    if (workspaceLockFile) {
      lockPath = (0, import_path6.join)(rootDir, workspaceLockFile.path);
    } else {
      ...
      lockPath = (0, import_path6.join)(projectDir, "uv.lock");
    }
  } else {
    projectDir = workPath;
    pyprojectPath = (0, import_path6.join)(projectDir, "pyproject.toml");
    ...
    const minimalManifest = (0, import_python_analysis4.createMinimalManifest)({
      name: "app",
      requiresPython,
      dependencies: []
    });
    const content = (0, import_python_analysis4.stringifyManifest)(minimalManifest);
    await import_fs5.default.promises.writeFile(pyprojectPath, content);
    await restoreCachedLock(cachedLockPath, projectDir);
    await uv.lock({
      projectDir,
  ...
  const resolvedLockPath = lockPath && import_fs5.default.existsSync(lockPath) ? lockPath : (0, import_path6.join)(projectDir, "uv.lock");
  return {
    projectDir,
    pyprojectPath,
...
async function pipInstall(pipPath, uvPath, workPath, args, targetDir) {
  const target = targetDir ? (0, import_path6.join)(targetDir, resolveVendorDir()) : resolveVendorDir();
  ...
  if (uvPath) {
    ...
    (0, import_build_utils6.debug)(`Running "${prettyUv}"...`);
    try {
      await (0, import_execa4.default)(uvPath, uvArgs, {
        cwd: workPath,
      ...
      (0, import_build_utils6.debug)(`error: ${err}`);
    }
  }
  const cmdArgs = [
    "install",
  ...
  (0, import_build_utils6.debug)(`Running "${pretty}"...`);
  try {
    await (0, import_execa4.default)(pipPath, cmdArgs, {
      cwd: workPath
    });
  } catch (err) {
...
async function installRequirementsFile({
  ...
  if (meta.isDev && await areRequirementsInstalled(pythonPath, filePath, actualTargetDir)) {
    (0, import_build_utils6.debug)(`Skipping requirements file installation, already installed`);
    return;
  }
  await pipInstall(
    pipPath,
...
smaller functions.`,
    ...
    let lockFile;
    try {
      lockFile = (0, import_python_analysis5.parseUvLock)(lockContent, relLockPath);
    } catch (error) {
      if (error instanceof import_python_analysis5.PythonAnalysisError) {
...
${error.fileContent}`
    ...
    const classification = (0, import_python_analysis5.classifyPackages)({
      lockFile,
      excludePackages
    });
    (0, import_build_utils7.debug)(
    ...
    const forceBundledSet = new Set(
      forceBundledDueToWheels.map(import_python_analysis5.normalizePackageName)
    );
    const externalizablePublic = classification.publicPackages.filter(
      (name) => !forceBundledSet.has((0, import_python_analysis5.normalizePackageName)(name))
    );
...
To fix this, either:
  ...
  2. Replace the packages that don't provide a pre-built wheel.`
    ...
    const externalizableSet = new Set(
      externalizablePublic.map(import_python_analysis5.normalizePackageName)
    );
    const publicPackageSizes = new Map(
      [...packageSizes].filter(
        ([name]) => externalizableSet.has((0, import_python_analysis5.normalizePackageName)(name))
      )
    );
    ...
    const bundledPublicSet = new Set(bundledPublic.map(import_python_analysis5.normalizePackageName));
    const externalizedPublic = externalizablePublic.filter(
      (name) => !bundledPublicSet.has((0, import_python_analysis5.normalizePackageName)(name))
    );
...
functions.`,
  ...
  async findPackagesWithoutCompatibleWheels(lockFile, publicPackageNames) {
    ...
    const relevantPackages = reachable ? publicPackageNames.filter(
      (name) => reachable.has((0, import_python_analysis5.normalizePackageName)(name))
    ) : publicPackageNames;
    ...
    const publicSet = new Set(relevantPackages.map(import_python_analysis5.normalizePackageName));
    const packageWheels = /* @__PURE__ */ new Map();
    for (const pkg of lockFile.packages) {
      const normalized = (0, import_python_analysis5.normalizePackageName)(pkg.name);
    ...
    for (const name of relevantPackages) {
      const normalized = (0, import_python_analysis5.normalizePackageName)(name);
      ...
      for (const wheel of wheels) {
        ...
        if (!filename || !filename.endsWith(".whl"))
          continue;
        try {
          const compatible = await (0, import_python_analysis5.isWheelCompatible)(
            filename,
            this.pythonVersion.major,
            this.pythonVersion.minor,
            platform.osName,
...
async function getPackagesReachableOnPlatform(lockFile, projectName, pythonMajor, pythonMinor, sysPlatform, platformMachine) {
  if (!projectName)
    return null;
  const rootNormalized = (0, import_python_analysis5.normalizePackageName)(projectName);
  const packageMap = /* @__PURE__ */ new Map();
  for (const pkg of lockFile.packages) {
    packageMap.set((0, import_python_analysis5.normalizePackageName)(pkg.name), pkg);
  }
  ...
  async function enqueueDeps(pkg) {
    ...
    for (const dep of pkg.dependencies) {
      const normalized = (0, import_python_analysis5.normalizePackageName)(dep.name);
      if (visited.has(normalized))
        continue;
      if (dep.marker) {
        try {
          const compatible = await (0, import_python_analysis5.evaluateMarker)(
            dep.marker,
            pythonMajor,
            pythonMinor,
            sysPlatform,
...
async function generateProjectManifest({
  ...
  const allParsed = await (0, import_python_analysis6.parsePep508)(allRawDeps.map((d) => d.dep));
  for (let i = 0; i < allRawDeps.length; i++) {
    const parsed = allParsed[i];
    if (!parsed)
      continue;
    const { dep, scope } = allRawDeps[i];
    const normalized = (0, import_python_analysis6.normalizePackageName)(parsed.name);
  ...
  const transitiveEntries = [];
  {
    ...
    const uvLock = (0, import_python_analysis6.parseUvLock)(content, uvLockPath);
    const projectName = project?.name;
    const excludeSet = new Set(
      projectName ? [(0, import_python_analysis6.normalizePackageName)(projectName)] : []
    );
    const lockMap = /* @__PURE__ */ new Map();
    const depGraph = /* @__PURE__ */ new Map();
    for (const pkg of uvLock.packages) {
      const normalized = (0, import_python_analysis6.normalizePackageName)(pkg.name);
      ...
      if (pkg.dependencies) {
        ...
        for (const d of pkg.dependencies) {
          deps.add((0, import_python_analysis6.normalizePackageName)(d.name));
        }
...
async function checkEntrypoint(workPath, relPath) {
  ...
  return (0, import_python_analysis7.findAppOrHandler)(content);
}
...
async function findEntrypointCandidates(workPath) {
  ...
  for (const relPath of pyFiles) {
    try {
      ...
      const varName = await (0, import_python_analysis7.findAppOrHandler)(content);
      if (varName) {
...
async function resolveExistingEntrypoint(workPath, filePath) {
  ...
  for (const candidate of candidates) {
    try {
      const stat = await import_fs10.default.promises.stat((0, import_path10.join)(workPath, candidate));
      if (stat.isFile()) {
        return candidate;
      }
    } catch {
...
var SUBSCRIBER_FIELD_NAMES = /* @__PURE__ */ new Set(["entrypoint", "topics"]);
var LEGACY_TRIGGER_NUMBER_FIELDS = [
  {
...
var LEGACY_SUBSCRIBER_FIELD_NAMES = /* @__PURE__ */ new Set([
  "entrypoint",
  "topics",
...
async function getPyprojectSubscribers(workPath, { legacySchema = false } = {}) {
  ...
  const parsedSubscribers = await Promise.all(
    subscribers.map(
      (config, index) => legacySchema ? parseLegacySubscriber(workPath, index, config) : parseSubscriber(workPath, index, config)
    )
  );
  const seenNames = /* @__PURE__ */ new Set();
...
function filterQueueSubscriptions(declaration, subscriptions) {
  if (!declaration.topicPatterns) {
    return subscriptions;
  }
  return subscriptions.filter(
    (subscription) => declaration.topicPatterns.some(
      (pattern) => queueTopicPatternsOverlap(pattern, subscription.topic)
    )
  );
}
...
function queueTopicPatternsOverlap(left, right) {
  ...
  if (leftPrefix === void 0 && rightPrefix === void 0) {
    return left === right;
  }
  if (leftPrefix !== void 0 && rightPrefix !== void 0) {
    return leftPrefix.startsWith(rightPrefix) || rightPrefix.startsWith(leftPrefix);
  }
  if (leftPrefix !== void 0) {
...
function createIntegrationInstallLines(integrations, { serving, beforeImport }) {
  return integrations.filter(
    ...
    ...serving && servingActivator ? [`from ${module2} import ${servingActivator}`, `${servingActivator}()`] : []
  ]);
}
...
async function parseLegacySubscriber(workPath, index, config) {
  const label = `subscriber #${index + 1}`;
  if (!config || typeof config !== "object" || Array.isArray(config)) {
    throw subscriberError(`${label} must be an object`);
  }
...
function parseLegacyTopics(name, value) {
  ...
  try {
    const { stdout } = await uv.run({
      ...
      env: {
        ...
        VERCEL_DEPLOYMENT_ID: process.env.VERCEL_DEPLOYMENT_ID || "dpl_introspection"
      }
    });
...
function parseIntrospectedSubscriptions(kind, subscriberName, stdout) {
  let parsed;
  try {
    parsed = JSON.parse(stdout);
  } catch {
...
function parseIntrospectedSubscription(kind, subscriberName, index, value) {
  ...
  if (typeof subscription.topic !== "string" || subscription.topic.length === 0) {
    throw subscriberError(
      `${label} must define non-empty string field "topic"`
    );
  }
  if (typeof subscription.consumer_group !== "string" || subscription.consumer_group.length === 0) {
    throw subscriberError(
...
async function isClientGenerated(pythonPath, env) {
  try {
    await (0, import_execa7.default)(pythonPath, ["-c", "import prisma.client"], {
      env,
      stdio: "pipe"
    });
    return true;
...
function toposortQuirks(activated) {
  ...
  for (const q of activated) {
    nameToQuirk.set((0, import_python_analysis10.normalizePackageName)(q.dependency), q);
  }
  ...
  for (const q of activated) {
    if (q.runsBefore) {
      for (const dep of q.runsBefore) {
        const target = nameToQuirk.get((0, import_python_analysis10.normalizePackageName)(dep));
        if (target) {
    ...
    if (q.runsAfter) {
      for (const dep of q.runsAfter) {
        const source = nameToQuirk.get((0, import_python_analysis10.normalizePackageName)(dep));
        if (source) {
...
async function runQuirks(ctx) {
  ...
  for (const dir of sitePackageDirs) {
    const distributions = await (0, import_python_analysis10.scanDistributions)(dir);
    for (const name of distributions.keys()) {
      installedNames.add((0, import_python_analysis10.normalizePackageName)(name));
    }
  }
...
function getDistributionFileGroups({
  ...
  const includeSet = includePackages ? new Set(includePackages.map(import_python_analysis11.normalizePackageName)) : null;
  const groups = [];
  for (const dir of sitePackageDirs) {
...
var InstalledPythonDistributions = class _InstalledPythonDistributions {
  static async load({
    ...
    for (const dir of sitePackageDirs) {
      ...
      distributions.set(dir, await (0, import_python_analysis11.scanDistributions)(dir));
    }
...
function getDevSubscriberTopics(subscriber) {
  if (subscriber.legacy) {
    const { retryAfterSeconds, initialDelaySeconds } = subscriber.legacy.triggerDefaults;
...
`);
  ...
  for (const subscriber of subscribers) {
    ...
    const experimentalTriggers = subscriber.subscriptions.map(
      (subscription) => ({
        type: "queue/v2beta",
        topic: subscription.topic,
        consumer: subscription.consumer,
  ...
  for (const workflow of workflows) {
    ...
    if (workflowMode === "queue") {
      ...
      (0, import_assert.default)(subscriptions, "workflow queue subscriptions must be resolved");
      const generatedHandlerPath = getGeneratedQueueHandlerPath(outputPath);
      const experimentalTriggers2 = subscriptions.map(
        (subscription) => ({
          type: "queue/v2beta",
          topic: subscription.topic,
          consumer: subscription.consumer,
      ...
      workflowLambdas[outputPath] = new import_build_utils24.Lambda({
        ...
        experimentalTriggers: experimentalTriggers2,
        supportsResponseStreaming: true
      });
      queueRoutePaths.push(outputPath);
      continue;
